...
# STEP1: Pack the datasets of backgrounds, only need SINGLE execution
# Need more than 8GB Memory, Close unnecessary programs in advance!
'''
dtd_dir = "dtd/images/"
bg_images = []
...
# TEST1: Test on one image for extracting
...
def extract_cards_from_video(video_fn, output_dir=None, keep_ratio=5, min_focus=20, debug=False):
    """
...
# TEST2: Test card extraction from a video
'''
imgs = extract_cards_from_video("test/strawberries.mp4", output_dir="test/strawberries/", debug=True)
...
extension = "mp4"
imgs_dir = "data/cards"
# STEP2: Extract images from video ,saving to data/cards, only need single execution
'''
for fruit in card_fruits:
    card_name = fruit
...
'''
# TEST3: Check if the bounding boxes are suitable.
'''
imgs_dir = "data/cards"
...
def findHull(img, corner=refCornerHL, debug="no"):
    ...
    for c in contours:
        ...
        cx = int(M['m10'] / M['m00'])
        cy = int(M['m01'] / M['m00'])
        # abs(w/2-cx)<w*0.3 and abs(h/2-cy)<h*0.4 : TWEAK, the idea here is to keep only the contours which are
        # closed to the center of the zone
    ...
    if concat_contour is not None:
        # At this point, we suppose that 'concat_contour' contains only the contours corresponding the value and suit
        #  symbols We can now determine the hull
        hull = cv2.convexHull(concat_contour)
        hull_area = cv2.contourArea(hull)
        # If the area of the hull is to small or too big, there may be a problem
        min_hull_area = 2000  # TWEAK, deck and 'zoom' dependant
        max_hull_area = 8000  # TWEAK, deck and 'zoom' dependant
...
# TEST4: Test find_hull on a random card image
'''
debug = "no"
imgs_dir = "data/cards"
imgs_fns = glob(imgs_dir + "/*/*.png")
...
# STEP3: Pack convex hulls information and images of cards.
'''
imgs_dir = "data/cards"
...
for fruit in card_fruits:
    ...
    print("Nb images for {card_name} : {len(cards[card_name])}")
...
# Load the cards pickle file in 'cards', check numbers of each fruit
...
# Generating a scene functions and classes
...
class Scene:
    ...
    def create2CardsScene(self, bg, img1, class1, hulla1, hullb1, img2, class2, hulla2, hullb2):
        ...
        while True:
            ...
            invalid = False
            intersect_ratio = 0.001  # tweak to adapt to different kinds of cards
            for i in range(1, 3):
                ...
                ai = intersect.area  # Center intersection area
                if ai != 0:  # Require two center parts can not intersect with each other
                    invalid = True
                    break
                else:
...
# TEST5: Test generation of a scene with 2 cards
'''
...
# STEP4: Generation of the 2 cards scenes
'''
nb_cards_to_generate = 1000
